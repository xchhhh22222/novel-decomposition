...
记录：平台、频道、题材、榜单类型、快照日期、目标样本数（默认10）、默认读取章数（3）、最多读取章数（10）、输出目录、用户授权范围与已知限制。
...
需要第11章以后、整本、EPUB 或桌面批量任务时，改用 `$fanqie-desktop-operator`，通过正常可见界面导入本次已确定的 book_id/URL、选择目录和格式、监控并核验导出文件。除非用户明确要求，不得为了开篇研究扩大到第10章以后；不得自动登录、绕过付费或使用未验证内部接口。
