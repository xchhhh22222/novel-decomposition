...
description: V1.1 素材调度型小说创造规划器：先把创作需求拆成素材槽位，再按波次调度正式套路卡与 DNA 组件，支持 V1.4 素材库的 faction/system/realm/technique/artifact/resource 组件级召回、来源回溯、兼容审计和跨书重组。
---
...
4. **建立素材调度单**：先确定共享素材库根目录，再按 `material-dispatch.md` 把创作需求拆成 required/optional 槽位，并按 Wave 1→4 调度。先骨架、再冲突供给、再表现层、最后验证补洞。V1.1 对世界观/修炼体系优先使用组件级检索，不再把整条 per_book 当作唯一召回单位。
5. **执行组件级召回与下钻**：每槽先取少量候选；需要具体势力/体系/功法/法宝/资源时使用 `search_dna_candidates.py --components ...`。短名单必须回查来源 `record_id / book_id / qa_status / evidence`，并执行来源集中度和兼容性审核。
6. **构造完整创作内核**：按“读者承诺→主角欲望与矛盾→世界问题→修炼与资源验证→金手指介入→人物关系改变选择→重复故事循环→阶段刷新”建立闭环。候选方案必须给出可直接讨论的正式名称和明确规则，不得只写“某系统”“某组织”“待定能力”等空占位。
7. **生成三案**：先把可迁移的世界规则、现成金手指母型、人物关系发动机与第一/第二大高潮组装成 A/B/C 三条不同故事线。每案说明来源与覆盖章数、主角要什么、谁阻碍、第一高潮的可见结果、第二高潮如何由前一结果引出、女主与反派各自怎样主动行动。推荐案由本 Agent 主动提出，不要求用户先发明核心剧情。三案可保留成熟金手指机制或做轻微改造；原创性审核重点在整体组合、人物、表达和事件链，不把大幅创新当成默认目标。
8. **分阶段审核**：选题三案先检查事实、素材证据、开篇匹配、约50章持续性、人物主动性与原创重设；只有用户要求完整长篇计划时才检查300章持续性并按100分制评分。80分门槛不阻止展示尚待选择的候选案。
9. **按用户所处阶段交付**：讨论选题时先交付三条有因果闭环的故事方向与推荐理由，不抢先铺 300 章或逐章细纲。用户选定第一大高潮后，以高潮所需的身份、战力、关系和信息状态倒推前约 50 章；选定第二大高潮后再设计后续阶段。只有用户要求完整开书包时，才展开黄金三章、前 30/50 章和长线滚动规划。
10. **输出覆盖与缺口**：生成素材覆盖矩阵和素材缺口订单。缺口订单交给独立拆书/入库流水线，本 Skill 不自行补库。
...
- 不允许先写方案再反向给素材贴标签。必须先建立素材槽位与检索记录，再形成候选组合。
