...
description: V1.4 统筹新书榜与共享素材库，将不同来源的世界规则/势力、修炼体系、境界、功法、法宝、资源、金手指、人物和高潮组件重新组合为新的 A/B/C 开书候选；不修改来源拆书记录。
---
...
4. **先从素材组装候选**：先确定共享素材库根目录。V1.4 允许按组件跨书选材：世界规则/factions、1～N 套 cultivation systems、realm、technique、artifact、resource 可以来自不同来源；同时检索金手指、人物个体/关系、高潮脉络和正式套路卡。DNA 检索按模块分批使用 `--include-per-book`，读取原记录与 QA/章节范围。组合后必须建立新的兼容层并标记为【新书候选】，禁止写回来源 `per_book`。
