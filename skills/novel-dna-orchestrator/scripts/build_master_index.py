...
def count_candidates(root: Path) -> tuple[Counter, dict[str, list[str]]]:
    counts: Counter = Counter()
    paths_by_book: dict[str, list[str]] = {}
    seen: set[tuple[str, str]] = set()
    if not root.exists():
        return counts, paths_by_book
    for path in root.rglob("*.jsonl"):
        ...
        for number, raw in enumerate(path.read_text(encoding="utf-8-sig").splitlines(), 1):
            ...
            for book_id in book_ids:
                ...
                if book_id and key not in seen:
                    counts[str(book_id)] += 1
                    paths_by_book.setdefault(str(book_id), []).append(str(path.relative_to(root)))
                    seen.add(key)
    return counts, paths_by_book


def count_derived(root: Path, record_type: str) -> tuple[Counter, dict[str, list[str]], dict[str, str]]:
    counts: Counter = Counter()
    paths_by_book: dict[str, list[str]] = {}
    coverage: dict[str, str] = {}
    if not root.exists():
        return counts, paths_by_book, coverage
    for path in root.rglob("*.jsonl"):
        for raw in path.read_text(encoding="utf-8-sig").splitlines():
            if not raw.strip():
                continue
            ...
            if not book_id:
                continue
            if row.get("record_type") == "derived_gap" and row.get("view_type") == record_type:
                coverage[book_id] = str(row.get("coverage_status", "insufficient_evidence"))
                continue
            if row.get("record_type") != record_type or row.get("status") != "candidate":
                continue
            if row.get("qa_status") != "PASS":
                coverage[book_id] = "insufficient_evidence"
                continue
            counts[book_id] += 1
            paths_by_book.setdefault(book_id, []).append(str(path.relative_to(root.parents[2])))
            coverage[book_id] = "ready"
    return counts, paths_by_book, coverage
...
def build_rows(library: Path) -> list[dict[str, object]]:
    ...
    candidate_counts, candidate_paths = count_candidates(library / "DNA素材")
    scopes = load_scopes(library / "DNA素材" / "00_任务清单")
    derived_roots = {
        "heroine": ("05_人物功能与标签/derived/heroine", "heroine_character"),
        "long_arc_villain": ("05_人物功能与标签/derived/long_arc_villain", "long_arc_villain"),
        "major_storyline": ("08_篇章结构/derived/major_storyline", "major_storyline"),
    }
    derived = {
        key: count_derived(library / "DNA素材" / folder, kind)
        for key, (folder, kind) in derived_roots.items()
    }
        ...
        rows.append(
            {
                ...
                "heroine_cards": derived["heroine"][0].get(book_id, 0),
                "heroine_paths": derived["heroine"][1].get(book_id, []),
                "heroine_coverage": derived["heroine"][2].get(book_id, "not_backfilled"),
                "long_arc_villain_cards": derived["long_arc_villain"][0].get(book_id, 0),
                "long_arc_villain_paths": derived["long_arc_villain"][1].get(book_id, []),
                "long_arc_villain_coverage": derived["long_arc_villain"][2].get(book_id, "not_backfilled"),
                "major_storyline_cards": derived["major_storyline"][0].get(book_id, 0),
                "major_storyline_paths": derived["major_storyline"][1].get(book_id, []),
                "major_storyline_coverage": derived["major_storyline"][2].get(book_id, "not_backfilled"),
...
def render_markdown(rows: list[dict[str, object]]) -> str:
    header = "| book_id | 书名 | 市场级 | QA | 章节范围 | 情绪可用/目标 | 情绪记录 | 主情绪 | 专项候选 | 女主卡 | 长线反派卡 | 大故事线 | 正式卡精确命中 | 缺口 | 导航 |\n|---|---|---|---|---|---|---:|---|---:|---|---|---|---:|---|---|"
    body = [
        f"| {r['book_id']} | {r['title']} | {r['market_grade']} | {r['qa_status']} | {r['chapters_covered']} | {r['emotion_coverage']} | {r['emotion_records']} | {r['dominant_emotion']} | {r['candidate_count']} | {r['heroine_cards'] or r['heroine_coverage']} | {r['long_arc_villain_cards'] or r['long_arc_villain_coverage']} | {r['major_storyline_cards'] or r['major_storyline_coverage']} | {r['active_card_count']} | {', '.join(map(str, r['gaps'])) or '-'} | DNA: {r['path']}; 候选: {'; '.join(r['candidate_paths']) or '-'}; 正式: {'; '.join(r['active_card_paths']) or '-'} |"
        for r in rows
    ]
    return "\n".join([header, *body]) + "\n"
