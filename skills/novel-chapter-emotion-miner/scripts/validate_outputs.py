...
def canonical_contract_drift() -> list[str]:
    ...
    upstream = Path(__file__).resolve().parents[3] / "skills" / "novel-dna-orchestrator" / "scripts" / "validate_chapter_emotions.py"
...
def resolve_workspace_root() -> Path:
    ...
    candidates = [
        ...
        if (ancestor / "skills").is_dir() and (ancestor / "素材库").is_dir()
    ]
    if len(candidates) != 1:
        raise RuntimeError(
            "WORKSPACE_ROOT_UNRESOLVED: expected exactly one ancestor containing both skills/ and 素材库/"
        )
    return candidates[0]
...
def recompute_source_fingerprint(row: dict[str, Any], project_root: Path) -> tuple[str | None, str | None]:
    ...
    source_path = project_root / Path(source_ref)
    try:
